...
export type ProductDto = { id: string; name: string; categoryId: string; category: Pick<CategoryDto, 'id' | 'name'>; priceVnd: number; isActive: boolean; sortOrder: number; optionGroupIds: string[]; optionGroups: OptionGroupDto[]; createdAt?: string; updatedAt?: string }
export type ProductFilters = { categoryId?: string; search?: string; isActive?: boolean }
export type ProductMutationDto = { name?: string; categoryId?: string; priceVnd?: number; isActive?: boolean; sortOrder?: number; optionGroupIds?: string[] }
