    ...
    <article className="bg-surface-container-lowest border border-outline-variant/50 rounded-lg p-3 shadow-sm" aria-label={`Món ${item.productNameSnapshot}`}>
      ...
      <div className="flex items-center gap-1 mt-1 flex-wrap">
        ...
        {item.options.map((option) => (
          <span key={option.optionId} className="rounded-full bg-surface-container-high px-2 py-0.5 text-xs text-on-surface-variant">
      ...
      {isEditingNote && (
        <div className="mt-2 pt-2 border-t border-outline-variant/50">
          <textarea id={`note-${item.tempId}`} aria-label="Ghi chú món" value={noteDraft} maxLength={200} onChange={(event) => setNoteDraft(event.target.value)} className="w-full bg-surface-container-low border border-outline-variant/50 rounded p-2 text-sm focus:ring-1 focus:ring-primary outline-none" rows={2} placeholder="Nhập ghi chú mới..." />
